...
import { faLayerGroup, faPlus, faXmark, faSearch, faSort, faSortUp, faSortDown, faChevronLeft, faChevronRight, faTrash, faPen, faEllipsisVertical, faSpinner, faLock, faLockOpen, faEye, faEyeSlash } from '@fortawesome/free-solid-svg-icons';
...
const Groups = ({ user, theme, setNotification }) => {
    ...
    const [form, setForm] = useState({ group_name: '', description: '', strict: false, privacy: false })
    ...
    const resetForm = () => { setForm({ group_name: '', description: '', strict: false, privacy: false }); setErrors({}) }

    useEffect(() => { setSubmitted(false); setFormOpen(false); setEdit(false); setEditId(null); resetForm(); setTableData(groups) }, [groups])
    ...
    useEffect(() => { dispatch(getGroups({ type: 'video' })) }, [])
    useEffect(() => { if (selectedData?.length > 0) { dispatch(deleteMultipleGroups({ ids: selectedData, type: 'video' })); setSelectedData(null) } }, [selectedData])
    useEffect(() => { if (confirm) dispatch(deleteGroups({ id: deleteId, type: 'video' })) }, [confirm])

    const editMode = (data) => {
        setForm({ group_name: data.group_name || '', description: data.description || '', strict: data.strict || false, privacy: data.privacy || false })
        setEditId(data._id); setEdit(true); setFormOpen(true)
    }

    const validateForm = () => {
        const errs = {}
        if (!form.group_name || form.group_name.length < 3) errs.group_name = form.group_name ? 'Must be at least 3 characters' : 'Required'
    ...
    const handleSubmit = (e) => {
        ...
        setSubmitted(true)
        if (edit) {
            dispatch(updateGroups({ data: { ...form, id: editId, type: 'video' } }))
        } else {
            dispatch(newGroups({ id: user._id, data: { ...form, user: user._id, type: 'video' } }))
        }
    }
    ...
    const filtered = useMemo(() => {
        ...
        return arr.filter(r => (r.group_name || '').toLowerCase().includes(q) || (r.description || '').toLowerCase().includes(q))
    }, [tableData, search])
    ...
    return (
        <div className="w-full px-4 sm:px-6 lg:px-8 py-6">
            <ConfirmModal theme={theme} title="Confirm Deletion" description="Are you sure you want to delete this group?" openModal={openModal} setOpenModal={setOpenModal} setConfirm={setConfirm} />
                ...
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isLight ? 'bg-violet-50' : 'bg-violet-900/20'}`}>
                            <FontAwesomeIcon icon={faLayerGroup} className={`text-xs ${isLight ? 'text-violet-500' : 'text-violet-400'}`} />
                        </div>
                        <h1 className={`text-2xl font-semibold ${isLight ? light.heading : dark.heading}`}>Groups</h1>
                    </div>
                    <p className={`text-sm ${isLight ? 'text-slate-500' : 'text-gray-500'}`}>Organize your videos into groups and collections</p>
                </div>
                <button onClick={() => { if (formOpen) { setFormOpen(false); resetForm(); setEdit(false) } else { setFormOpen(true); resetForm(); setEdit(false) } }}
                    className={`py-2 px-4 rounded-lg text-xs font-medium flex items-center gap-2 transition-all ${formOpen ? (isLight ? 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50' : 'bg-[#1C1C1C] border border-[#333] text-gray-400 hover:bg-[#222]') : (isLight ? 'bg-violet-500 text-white hover:bg-violet-600 shadow-sm' : 'bg-violet-600 text-white hover:bg-violet-700')}`}>
                ...
                <form onSubmit={handleSubmit} className="max-w-2xl space-y-5 mb-8">
                    <div className={panelClass}>
                        <div className={`px-5 py-3 border-b ${isLight ? 'border-slate-100' : 'border-[#222]'}`}>
                            <p className={sectionLabel}>{edit ? 'Edit Group' : 'New Group'}</p>
                        </div>
                        <div className="p-5 space-y-5">
                            <div>
                                <label className={labelClass}>Group Name</label>
                                <input type="text" value={form.group_name} onChange={e => setForm(p => ({ ...p, group_name: e.target.value }))} placeholder="Enter group name" className={inputClass} />
                                {errors.group_name && <p className="text-xs text-red-500 mt-1">{errors.group_name}</p>}
                            </div>
                            <div>
                                <label className={labelClass}>Description</label>
                                <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Describe this group..." rows={3} className={`${inputClass} resize-none`} />
                    ...
                    <div className={panelClass}>
                        ...
                        <div className="p-5 space-y-4">
                            {[{ name: 'strict', label: 'Strict Mode', desc: 'Enable content restrictions' }, { name: 'privacy', label: 'Private', desc: 'Only visible to you' }].map(({ name, label, desc }) => (
                                <label key={name} className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all ${isLight ? 'hover:bg-slate-50' : 'hover:bg-[#1A1A1A]'}`}>
                                    <div><p className={`text-sm font-medium ${isLight ? 'text-slate-700' : 'text-gray-200'}`}>{label}</p><p className={`text-xs ${isLight ? 'text-slate-400' : 'text-gray-500'}`}>{desc}</p></div>
                    ...
                    <button type="submit" disabled={submitted} className={`w-full py-3 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-all ${submitted ? 'opacity-60 cursor-not-allowed' : ''} ${isLight ? 'bg-violet-500 text-white hover:bg-violet-600 shadow-sm' : 'bg-violet-600 text-white hover:bg-violet-700'}`}>
                        {submitted ? <><FontAwesomeIcon icon={faSpinner} className="animate-spin text-xs" /> Saving...</> : <>{edit ? 'Update Group' : 'Create Group'}</>}
                    </button>
                </form>
            )}
            ...
            {!formOpen && (
                <div className={`rounded-xl border overflow-hidden ${isLight ? 'bg-white/80 backdrop-blur-sm border-slate-200/60 shadow-sm' : 'bg-[#161616] border-[#2B2B2B]'}`}>
                    <div className={`px-5 py-4 flex items-center justify-between border-b ${isLight ? 'border-slate-100' : 'border-[#222]'}`}>
                        <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                                <div className="flex items-center gap-2"><h3 className={`text-sm font-semibold truncate ${isLight ? 'text-slate-800' : 'text-white'}`}>All Groups</h3><span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${isLight ? 'bg-violet-50 text-violet-500' : 'bg-violet-900/20 text-violet-400'}`}>{(tableData || []).length}</span></div>
                                <p className={`text-[11px] ${isLight ? 'text-slate-400' : 'text-gray-500'}`}>Manage your video groups</p>
                            </div>
                        </div>
                    ...
                    {loading ? (
                        <div className="flex items-center justify-center py-20"><div className={`animate-spin rounded-full h-7 w-7 border-2 border-t-transparent ${isLight ? 'border-violet-500' : 'border-violet-400'}`} /></div>
                    ) : (
                        <>
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead><tr className={isLight ? 'bg-slate-50/80' : 'bg-[#1A1A1A]'}>
                                    ...
                                    <tbody>
                                        {pageData.length > 0 ? pageData.map((row, i) => (
                                            <tr key={row._id || i} className={`border-t transition-colors ${isLight ? 'border-slate-100' : 'border-[#222]'} ${selected.includes(row._id) ? (isLight ? 'bg-blue-50/50' : 'bg-blue-900/10') : (i % 2 === 1 ? (isLight ? 'bg-slate-50/30' : 'bg-[#1A1A1A]/50') : '')} ${isLight ? 'hover:bg-blue-50/40' : 'hover:bg-[#0e0e0e]'}`}>
                                                ...
                                                <td className={tdClass}><span className={`text-xs font-medium px-2 py-1 rounded-md ${isLight ? 'bg-slate-100 text-slate-600' : 'bg-[#2B2B2B] text-gray-300'}`}>{row.items ?? 0}</span></td>
                                            ...
                                            <tr><td colSpan={columns.length + 1} className={`px-4 py-16 text-center ${isLight ? 'text-slate-400' : 'text-gray-500'}`}><FontAwesomeIcon icon={faLayerGroup} className="text-2xl mb-3 opacity-20 block mx-auto" /><p className="text-sm font-medium">No groups found</p><p className={`text-xs mt-1 ${isLight ? 'text-slate-300' : 'text-gray-600'}`}>{search ? 'Try a different search' : 'Create your first group'}</p></td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                            {sorted.length > 0 && (
                                <div className={`px-5 py-3 flex items-center justify-between border-t ${isLight ? 'border-slate-100 bg-slate-50/50' : 'border-[#222] bg-[#141414]'}`}>
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => { const pn = totalPages <= 5 ? i : Math.min(Math.max(page - 2, 0), totalPages - 5) + i; return (<button key={pn} onClick={() => setPage(pn)} className={`w-7 h-7 rounded-md text-[11px] font-medium transition-all ${page === pn ? (isLight ? 'bg-blue-500 text-white shadow-sm' : 'bg-blue-600 text-white') : (isLight ? 'text-slate-500 hover:bg-slate-100' : 'text-gray-400 hover:bg-[#333]')}`}>{pn + 1}</button>) })}
